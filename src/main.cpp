...
#if defined(NDEBUG)
# error "Smcoins cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Smcoins Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Smcoins: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Smcoins: find block wanted by given orphan block
...
// Smcoins: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Smcoins miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Smcoins: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Smcoins: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Smcoins: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Smcoins: track money supply and mint amount info
...
// Smcoins: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Smcoins: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Smcoins: compute stake entropy bit for stake modifier
    ...
    // Smcoins: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Smcoins: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Smcoins: attempt to generate suitable proof-of-stake
